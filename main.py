...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Comma-separated A-share markets, such as main,gem,star,bse or the Chinese names.",
    )
    parser.add_argument(
        "--tushare-exchanges",
        type=str,
...
def main() -> None:
    ...
    if args.data_source == "csv":
        ...
        data_description = f"loaded from {Path(args.prices).resolve()}"
        data_metadata = {"data_source": "csv", "path": str(Path(args.prices).resolve())}
        ...
        data_description = "generated from the built-in synthetic data generator"
        data_metadata = {
            "data_source": "demo",
        ...
        prices = download_tushare_prices(
            token=token,
            tickers=symbols,
            start_date=start_date,
            end_date=end_date,
        ...
        data_description = (
            f"downloaded from Tushare daily API for {len(symbols)} A-share symbols "
            f"between {start_date} and {end_date}"
        )
        data_metadata = {
            "data_source": "tushare",
        ...
        if universe is not None:
            data_metadata["auto_universe_size"] = len(universe)
            data_metadata["auto_universe_preview"] = universe[
                ["ts_code", "name", "market", "amount"]
            ].head(20).to_dict(orient="records")

    effective_min_assets = min(args.min_assets, prices.shape[1])
